...
char* encryptedRailFence(char* text, int key) {
    // create the matrix of cipher plain text
    ...
    // filling the rail to distinguish filled spaces from blank ones.
    for (i=0; i < key; i++) 
...
char* decryptRailFence(char* cipher, int key) {
    ...
    for (i=0; i<strlen(cipher); i++) {
        ...
        //place the marker 
        rail[row][col++] = '*';
        // find the next row using the direction of flag;
        dir_down?row++ : row--;

    }

    // now we can construct the fill rail matric 
    for (i=0; i<key; i++) 
        for (j=0; j<strlen(cipher); j++) 
...
// driver prgram to check the above functions 
int main() {
    char plain_text[50];
    ...
    scanf("%d", &key);
    cipher_text = encryptedRailFence(plain_text, key);
